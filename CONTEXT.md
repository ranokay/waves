...
**Audio quality**:
The fidelity tier a download is fetched at, stated on Waves' own four-rung ladder (LOW < HIGH < LOSSLESS < HI*RES_LOSSLESS), serialized as the ladder's tier strings. Audio type is orthogonal to it. Each provider maps its engine's codecs onto the rungs (e.g. AAC 320 → HIGH, ALAC 24/192 → HI_RES_LOSSLESS).
\_Avoid*: bitrate, resolution
