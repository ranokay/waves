...
**Chooser**:
The per-download control where the user picks audio quality, audio type, and lyrics/art options; the provider is the row's own, stated as a static chip rather than picked. Its defaults come from Settings, and one click uses those defaults.
