...
Item {
  ...
  // Console button spec, mirrors Main.qml.
  ...
  Timer {
    id: flashTimer
    interval: 2200
    onTriggered: page.savedFlash = false
  }
  Timer {
    id: auUpToDateTimer
    interval: 4000
  ...
  function ffSyncEverMissing() {
    if (page.ff && page.ff.stateKey === "missing")
      page.ffEverMissing = true
  }
  Connections {
    target: page.ff
    function onStateKeyChanged() {
      page.ffSyncEverMissing()
    }
  }
  ...
  function libTruncationNote() {
    ...
    return (measured ? ("This folder does not list properly. The system offers " + entries + " entries for it but only " + distinct + " of them are different folders, so the rest never reach Waves. ") : "This folder's listing comes back incomplete, which some network shares do. ") + "Everything inside the folders it did name is indexed. Waves finds the others by asking for " + "artists by name, which it does whenever you search, open a page, or start a download, and " + "what it finds is kept. Artists you have never looked up may show no badge yet."
  }
  ...
  readonly property bool libTruncationShown: libraryScanPartial && !libraryListingReconciled && libraryScanStatus === "ok"
  ...
  readonly property bool libScanReading: libraryScanStatus === "scanning" && (libraryScanProgress || {}).phase === "read" && (libraryScanProgress || {}).total > 0
  ...
  Connections {
    ...
    function onAppleRuntimeStateChanged(state, msg) {
      page.appleRuntimeState = state
      page.appleRuntimeMsg = msg
      page.appleSetupLive = waves.appleSetupState()
    }
    function onAppleRuntimeProgress(pct) {
      page.appleRuntimePct = pct
    }
  ...
  function auRefresh() {
    ...
    if (page.appUp.pending_restart === true && page.auState === "") {
      page.auState = "done"
      page.auMsg = (page.appUp.pending_version ? "Update to " + page.appUp.pending_version + " is ready. " : "An update is ready. ") + "Restart to finish."
    }
  }
  ...
  function auCheck() {
    if (page.auBusy || page.auChecking)
      return
  ...
  function val(f) {
    return editMap[f.key] !== undefined ? editMap[f.key] : f.value
  }
  // Secondary value for a composite field (the cover_sizes "separate file"
  // dropdown), read/written under its own file_key.
  function val2(f) {
    return editMap[f.file_key] !== undefined ? editMap[f.file_key] : f.file_value
  }
  // Child toggle value for the cover_scope composite, under its own child_key.
  function valChild(f) {
    return editMap[f.child_key] !== undefined ? editMap[f.child_key] : f.child_value
  }
  function setv(key, v) {
  ...
  function mapHas(f, ch) {
    return mapVal(f)[ch] !== undefined
  }
  function mapText(f, ch) {
    var v = mapVal(f)[ch]
  ...
  function mapIsDefault(f) {
    ...
    var cur = mapVal(f)
    for (var k in d)
      if (cur[k] === undefined || String(cur[k]) !== String(d[k]))
        return false
    for (var c in cur)
      if (d[c] === undefined)
        return false
  ...
  function mapDefaultChars(f) {
    ...
    for (var i = 0; i < chars.length; i++)
      if (d[chars[i].char] !== undefined)
        out.push(chars[i].char)
    return out
  }
  ...
  function hasInvalidEdits() {
    for (var k in page.sanitizeKeys) {
      if (sanitizeDirty(page.sanitizeKeys[k]))
        return true
    }
    return false
  }
  ...
  function boolFields(fields) {
    return fields.filter(function (f) {
      return f.type === "bool" && f.embedded !== true
    })
  }
  function rowFields(fields) {
    return fields.filter(function (f) {
      return f.type !== "bool" && f.embedded !== true && f.third !== true && f.type !== "char_map"
    })
  }
  ...
  function thirdFields(fields) {
    return fields.filter(function (f) {
      return f.type !== "bool" && f.embedded !== true && f.third === true
    })
  }
  ...
  function urlToPath(u) {
    ...
    if (s === "localhost" || s.indexOf("localhost/") === 0)
      s = s.substring(9) || "/"
  ...
  function iconPath(id) {
    switch (id) {
    case "downloads":
      return "M8 2.5V9.3 M5.2 6.6 L8 9.4 L10.8 6.6 M3.4 12.6H12.6"
    // Provider sections show their descriptors' logos instead of line
    // art, so they carry no iconPath case here.
    case "files":
      return "M2.6 5.4H6.2L7.4 6.7H13.4V11.9H2.6Z"
    case "metadata":
      return "M8.6 2.6H3.1V8L9 13.9L14.4 8.5Z M5.7 4.9A0.55 0.55 0 1 1 4.6 4.9A0.55 0.55 0 1 1 5.7 4.9Z"
    case "processing":
      return "M5.2 5.2H10.8V10.8H5.2Z M6.7 5.2V3.6 M9.3 5.2V3.6 M6.7 10.8V12.4 M9.3 10.8V12.4 M5.2 6.7H3.6 M5.2 9.3H3.6 M10.8 6.7H12.4 M10.8 9.3H12.4"
    case "discography":
      return "M2.6 8A5.4 5.4 0 1 0 13.4 8A5.4 5.4 0 1 0 2.6 8Z M6.9 8A1.1 1.1 0 1 0 9.1 8A1.1 1.1 0 1 0 6.9 8Z"
    // Book spines on a shelf, one leaning: the section never had its own
    // glyph and fell through to the generic sliders.
    case "library":
      return "M2.8 12.6H13.2 M4.4 4V12.6 M7 4V12.6 M9.4 4.6L11.8 12.6"
    case "updates":
      return "M8 3V12.4 M4.6 6.4L8 3L11.4 6.4"
    // Pulse/heartbeat trace: diagnostics watch the app's vitals.
    case "diagnostics":
      return "M2.6 8H5.4L6.8 4.6L9.2 11.4L10.6 8H13.4"
    default:
      return "M3 5.4H13 M3 10.6H13 M5.4 5.4A1.5 1.5 0 1 0 8.4 5.4A1.5 1.5 0 1 0 5.4 5.4Z M7.6 10.6A1.5 1.5 0 1 0 10.6 10.6A1.5 1.5 0 1 0 7.6 10.6Z"
    }
  }
  ...
  Component.onCompleted: {
    ...
    refreshSchema()
    auRefresh()
    waves.startupUpdateCheck()
    waves.startupFfmpegUpdateCheck()
  }
  ...
  Connections {
    ...
    function onAppUpdateProgress(pct) {
      page.auPct = pct
    }
    function onAppUpdateStatusChanged() {
      page.auRefresh()
    }
  ...
  component SToggle: Rectangle {
    ...
    border.color: checked ? page.accent : page.outline
    border.width: 2
  ...
  component SCombo: ComboBox {
    ...
    delegate: ItemDelegate {
      ...
      background: Rectangle {
        color: highlighted ? page.surface3 : page.surface2
      }
  ...
  component SText: Rectangle {
    ...
    radius: 8
    color: invalid ? page.redCont : page.surface2
    ...
    Repeater {
      ...
      delegate: Row {
        ...
        Text {
          text: modelData
          textFormat: Text.PlainText
          color: modelData.indexOf("{") >= 0 ? page.gold : index === pv.path.split("/").length - 1 ? page.textLo : page.textDim
          font.family: page.mono
          font.pixelSize: 11
        }
        Text {
  ...
  component SectionIcon: Shape {
    ...
    ShapePath {
      strokeColor: si.stroke
      strokeWidth: 1.5
      fillColor: "transparent"
      capStyle: ShapePath.RoundCap
      joinStyle: ShapePath.RoundJoin
      PathSvg {
        path: page.iconPath(si.glyph)
      }
    }
  }
  ...
  ColumnLayout {
    ...
    Item {
      ...
      Row {
        ...
        spacing: 10
        Rectangle {
          width: cancelTxt.width + page.btnPadH * 2
          height: cancelTxt.height + page.btnPadV * 2
          radius: page.btnRad
          color: "transparent"
        ...
        Rectangle {
          ...
          width: saveTxt.width + page.btnPadH * 2
          height: saveTxt.height + page.btnPadV * 2
          radius: page.btnRad
          color: page.accentCont
    ...
    Flickable {
      ...
      onContentYChanged: if (height > 0 && page.active)
        page.keptY = contentY
      ...
      Column {
        ...
        Component {
          ...
          Item {
            ...
            implicitHeight: twin ? Math.max(ffLeftCol.implicitHeight + 28, ffTileR.rowImplicitHeight + 24, 108) : ffSingle.implicitHeight
              ...
              ColumnLayout {
                ...
                RowLayout {
                  ...
                  Rectangle {
                    ...
                    radius: 5
                    color: page.goldCont
                    border.color: page.gold
                    implicitWidth: updT.implicitWidth + 14
                    implicitHeight: 19
                    Text {
                      id: updT
                      textFormat: Text.PlainText
                  ...
                  Rectangle {
                    ...
                    Text {
                      ...
                      textFormat: Text.PlainText
                      anchors.centerIn: parent
                      text: (page.ff.stateKey === "managed" ? "MANAGED" : "SYSTEM") + (page.ff.status.version ? " · " + page.ff.status.version : "")
                ...
                Text {
                  ...
                  color: page.textLo
                  font.pixelSize: 13
                  text: page.ff.stateKey === "managed" ? "Installed and managed by Waves." : page.ff.stateKey === "path" ? "Using the FFmpeg already on your system." : "FFmpeg is not installed."
                }
                Text {
                  Layout.fillWidth: true
                  wrapMode: Text.WordWrap
                ...
                Rectangle {
                  ...
                  Layout.alignment: Qt.AlignHCenter
                  Layout.topMargin: 2
                  Layout.preferredHeight: ffInstTxt.implicitHeight + page.btnPadV * 2
                  implicitWidth: ffInstTxt.implicitWidth + page.btnPadH * 2
                  radius: page.btnRad
                  color: page.accentCont
                  border.width: 1
                  border.color: page.accentDim
                ...
                ColumnLayout {
                  ...
                  RowLayout {
                    ...
                    ColumnLayout {
                      ...
                      spacing: 2
                      Text {
                        text: "Let Waves manage FFmpeg instead"
                        color: page.textHi
                        font.pixelSize: 13
                        font.weight: Font.Medium
                        Layout.fillWidth: true
                    ...
                    Rectangle {
                      Layout.alignment: Qt.AlignVCenter
                      implicitWidth: ffMgdTxt.implicitWidth + page.btnPadH * 2
                      implicitHeight: ffMgdTxt.implicitHeight + page.btnPadV * 2
                      radius: page.btnRad
                ...
                Rectangle {
                  ...
                  Layout.alignment: Qt.AlignHCenter
                  Layout.topMargin: 2
                  Layout.preferredHeight: ffCanTxt.implicitHeight + page.btnPadV * 2
                  implicitWidth: ffCanTxt.implicitWidth + page.btnPadH * 2
                  radius: page.btnRad
                  color: "transparent"
                  border.color: page.red
                  Text {
                    id: ffCanTxt
                    anchors.centerIn: parent
                    text: "CANCEL"
              ...
              ColumnLayout {
                ...
                RowLayout {
                  ...
                  Text {
                    ...
                    text: page.ff.updateAvailable ? "update available" : "installed and managed by Waves"
                  }
                  Rectangle {
                    visible: page.ff.updateAvailable && !page.ff.busy
                    Layout.alignment: Qt.AlignVCenter
                    ...
                    color: page.goldCont
                    border.color: page.gold
                    implicitWidth: ffUpdT.implicitWidth + 14
                    implicitHeight: 19
                    Text {
                      id: ffUpdT
                      textFormat: Text.PlainText
                  ...
                  Rectangle {
                    ...
                    radius: 5
                    implicitHeight: 19
                    implicitWidth: ffChipTxt.implicitWidth + 16
                    color: page.green
                    Text {
                      id: ffChipTxt
                      textFormat: Text.PlainText
                ...
                RowLayout {
                  ...
                  Rectangle {
                    id: ffPrimary
                    readonly property string label: page.ff.busy ? "Updating…" : page.ff.updateAvailable ? "Update" : (page.ff.checking ? "Checking…" : "Check for updates")
                    implicitWidth: ffPrimTxt.implicitWidth + page.btnPadH * 2
                    implicitHeight: ffPrimTxt.implicitHeight + page.btnPadV * 2
                    radius: page.btnRad
                    ...
                    MouseArea {
                      ...
                      onClicked: {
                        if (page.ff.updateAvailable)
                          ...
                          page.ff.checkUpdates()
                      }
                    }
                  }
                  Rectangle {
                    visible: !page.ff.busy
                    implicitWidth: ffRemTxt.implicitWidth + page.btnPadH * 2
                    implicitHeight: ffRemTxt.implicitHeight + page.btnPadV * 2
                    radius: page.btnRad
                    color: page.redCont
                    border.color: Qt.alpha(page.red, 0.55)
                    Text {
                      id: ffRemTxt
                      anchors.centerIn: parent
                      text: "REMOVE"
                  ...
                  Rectangle {
                    visible: page.ff.busy
                    implicitWidth: ffCanTxt2.implicitWidth + page.btnPadH * 2
                    implicitHeight: ffCanTxt2.implicitHeight + page.btnPadV * 2
                    radius: page.btnRad
                    color: "transparent"
                    border.color: page.red
                    Text {
        ...
        Component {
          ...
          Item {
            ...
            readonly property bool canInstall: page.appUp.can_self_install === true || page.appUp.can_managed_install === true
              ...
              ColumnLayout {
                ...
                RowLayout {
                  ...
                  Text {
                    ...
                    text: auCard.st === "not_configured" ? "automatic updates aren't available in this build" : auCard.st === "source" ? ("from source" + (page.auUpdate ? " · v" + page.auLatest + " available" : " · up to date")) : auCard.st === "managed" ? ("via " + (page.appUp.channel_label || "a package manager") + (page.auUpdate ? " · v" + page.auLatest + " available" : " · up to date")) : (page.auUpdate ? "v" + page.auLatest + " available" : (page.auDone ? "installed; restart to finish" : "up to date"))
                  }
                  Rectangle {
                    ...
                    color: "#2a2008"
                    border.color: page.gold
                    implicitWidth: auBadge.implicitWidth + 14
                    implicitHeight: 18
                    Text {
                      id: auBadge
                      anchors.centerIn: parent
                ...
                Text {
                  Layout.fillWidth: true
                  wrapMode: Text.WordWrap
                  text: auCard.st !== "managed" ? "Checks the public releases page and only notifies you; sends none of your data." : auCard.canInstall ? ("This copy is managed by " + (page.appUp.channel_label || "a package manager") + "; Update & restart runs its upgrade" + (page.appUp.update_hint ? " (" + page.appUp.update_hint + ")" : "") + " for you.") : ("This copy updates through " + (page.appUp.channel_label || "its package manager") + (page.appUp.update_hint ? " (" + page.appUp.update_hint + ")" : "") + "; checks here only notify you.")
                  color: page.textDim
                  font.pixelSize: 12
                }

                // Progress (while downloading/installing): the shared
                // LED dot-matrix pill (LedBar.qml, same as the DownIcon
                // running style).
                LedBar {
                  visible: page.auBusy
                  Layout.fillWidth: true
                ...
                RowLayout {
                  ...
                  Rectangle {
                    ...
                    // Check is a lit primary too: the section's one
                    // action shouldn't read as a dim outline next to
                    // lit toggles.
                    readonly property bool accent: restartMode || updateMode || checkMode
                    readonly property string label: page.auBusy ? "Updating…" : restartMode ? "Restart now" : updateMode ? "Update & restart" : browseMode ? "Open releases page" : checkMode ? (page.auChecking ? "Checking…" : "Check for updates") : ""
                    visible: label !== ""
                    implicitWidth: auPrimTxt.implicitWidth + page.btnPadH * 2
                    implicitHeight: auPrimTxt.implicitHeight + page.btnPadV * 2
                    radius: page.btnRad
                    ...
                    MouseArea {
                      ...
                      onClicked: {
                        ...
                        else if (auPrimary.browseMode)
                          waves.openReleasesPage()
                        else if (auPrimary.checkMode)
                          page.auCheck()
                      }
                    }
                  }
                  ...
                  Rectangle {
                    visible: page.auBusy
                    implicitWidth: auCanTxt.implicitWidth + page.btnPadH * 2
                    implicitHeight: auCanTxt.implicitHeight + page.btnPadV * 2
                    radius: page.btnRad
                    color: "transparent"
                    border.color: page.red
                    Text {
            ...
            Rectangle {
              ...
              ColumnLayout {
                ...
                RowLayout {
                  ...
                  Rectangle {
                    id: dgPrimary
                    implicitWidth: dgPrimTxt.implicitWidth + page.btnPadH * 2
                    implicitHeight: dgPrimTxt.implicitHeight + page.btnPadV * 2
                    radius: page.btnRad
                  ...
                  Rectangle {
                    ...
                    implicitWidth: dgShowTxt.implicitWidth + page.btnPadH * 2
                    implicitHeight: dgShowTxt.implicitHeight + page.btnPadV * 2
                    radius: page.btnRad
                    color: "transparent"
                    border.color: page.outline
                    Text {
          ...
          delegate: Rectangle {
              ...
              RowLayout {
                ...
                Rectangle {
                  ...
                  Row {
                    ...
                    visible: glyphTile.dualLogo
                    Repeater {
                      model: (card.modelData.providers !== undefined && glyphTile.dualLogo) ? card.modelData.providers : []
                      delegate: Image {
                        required property var modelData
                        anchors.verticalCenter: parent.verticalCenter
                        ...
                        width: modelData.logo_header_width !== undefined ? Number(modelData.logo_header_width) : 14
                        height: modelData.logo_header_height !== undefined ? Number(modelData.logo_header_height) : 14
                ...
                Rectangle {
                  ...
                  Layout.preferredWidth: cntT.implicitWidth + 16
                  Layout.alignment: Qt.AlignVCenter
                  Text {
                    id: cntT
                    anchors.centerIn: parent
            ...
            Item {
              ...
              Column {
                    ...
                    Column {
                      ...
                      Row {
                        ...
                        Text {
                          ...
                          color: !changed ? page.textDim : mapDefaultMa.containsMouse ? page.accent : page.accentDim
                      ...
                      Rectangle {
                        id: offerStrip
                        visible: mapCard.modelData.offer === true && !mapCard.offerAnswered && Object.keys(page.mapVal(mapCard.modelData)).length === 0
                        width: parent.width
                        ...
                        Column {
                          ...
                          Text {
                            ...
                            wrapMode: Text.WordWrap
                            textFormat: Text.PlainText
                            text: "Waves can write these five characters instead of removing them. " + "Folders already on disk keep the names they have; this applies to new downloads."
                            color: page.textDim
                            font.pixelSize: 12
                          }
                        ...
                        Repeater {
                          ...
                          delegate: Row {
                            ...
                            SText {
                              ...
                              placeholderText: charRow.overridden ? "none" : (page.generalStandIn() === "" ? "none" : page.generalStandIn())
                              invalid: page.mapCharDirty(charRow.fieldData, charRow.ch)
                              onEdited: function (t) {
                                page.mapSet(charRow.fieldData, charRow.ch, t)
                              }
                      ...
                      Loader {
                        ...
                        sourceComponent: Column {
                          ...
                          RowLayout {
                            ...
                            Rectangle {
                              ...
                              Text {
                                id: segDl
                                anchors.centerIn: parent
                                text: "My download folder"
                                color: !libraryCol.separate ? page.accent : page.textLo
                                font.pixelSize: 13
                            ...
                            SText {
                              ...
                              onEdited: function (t) {
                                page.setv("library_folder", t)
                              }
                            }
                            Rectangle {
                              visible: libraryCol.separate
                              Layout.preferredHeight: 36
                              implicitWidth: brwTxt.width + 28
                              radius: 8
                              color: page.surface2
                              border.color: page.border1
                              Text {
                            ...
                            Rectangle {
                              Layout.preferredHeight: 36
                              implicitWidth: startTxt.width + 28
                              radius: 8
                          ...
                          Text {
                            ...
                            width: parent.width
                            wrapMode: Text.WordWrap
                            text: libraryCol.libDirty && libraryCol.root !== "" ? "SAVE CHANGES applies this and starts the scan." : (libraryCol.separate ? "Choose a folder, then SAVE CHANGES." : "Set a download folder in Downloads, then SAVE CHANGES.")
                          ...
                          Text {
                            visible: (libraryCol.scanning && !page.libScanReading) || page.libraryScanStatus === "unreadable" || (page.libraryScanStatus === "missing" && libraryCol.root !== "") || (page.libTruncationShown && libraryCol.root !== "")
                            width: parent.width
                            wrapMode: Text.WordWrap
                            textFormat: Text.PlainText
                            font.pixelSize: 13
                            font.weight: Font.Medium
                            color: page.gold
                            text: libraryCol.scanning ? page.libScanNote() : page.libraryScanStatus === "unreadable" ? (Qt.platform.os === "osx" ? "Waves doesn't have permission to read this folder. It is likely on a network share or external drive that macOS protects. Allow access when macOS asks, or if you already declined, turn Waves on under System Settings, Privacy & Security, then reopen Settings." : "Waves doesn't have permission to read this folder. Check the folder's permissions (and, on a network share, that the mount allows this user to read it), then reopen Settings.") : page.libraryScanStatus === "missing" ? "This folder isn't available right now. If it lives on an external drive or NAS, connect it and reopen Settings." : page.libTruncationNote()
                          }
                          ...
                          Column {
                            ...
                            Item {
                              ...
                              Text {
                                ...
                                font.family: page.mono
                                color: page.textLo
                                text: (page.libraryScanProgress.artist || "?") + ((page.libraryScanProgress.album || "") !== "" ? " · " + page.libraryScanProgress.album : "")
                              }
                            }
                            ...
                            Rectangle {
                              ...
                              Text {
                                id: scanDotsPct
                                textFormat: Text.PlainText
                                text: page.libScanReading ? Math.floor(100 * page.libraryScanProgress.done / Math.max(1, page.libraryScanProgress.total)) + "%" : ""
                                color: page.accent
                                font.family: page.mono
                                font.pixelSize: 11
                                font.bold: true
                              ...
                              DotBar {
                                ...
                                anchors.left: parent.left
                                anchors.leftMargin: 12
                                anchors.right: scanDotsPct.left
                ...
                Repeater {
                  model: card.modelData.id === "files" ? page.rowFields(card.modelData.fields).filter(function (f) {
                    return f.key.indexOf("format_") === 0
                        ...
                        Repeater {
                          ...
                          delegate: Item {
                            ...
                            Timer {
                              id: tokCopiedTimer
                              interval: 1400
                              onTriggered: tokRow.copied = false
                            }
                ...
                Flow {
                  ...
                  Repeater {
                    ...
                    delegate: Rectangle {
                      ...
                      width: (flagFlow.flagCount % flagFlow.cols === 1 && flagTile.index === flagFlow.flagCount - 1) ? inner.width : (inner.width - 10 * (flagFlow.cols - 1)) / flagFlow.cols
                          ...
                          Item {
                            ...
                            RowLayout {
                              ...
                              Rectangle {
                                ...
                                border.color: on ? page.accent : page.outline
                                border.width: 2
                                Behavior on color {
                                  ColorAnimation {
                                    duration: 140
        ...
        Item {
          ...
          function visFor(off, len) {
            return Math.max(0, Math.min(footer.n - off, len))
          }
          function gibHeart() {
            if (footer.heartGibbed)
              return
            beatAnim.stop()
            beatReset.stop()
            footer.beat = 1.0
          ...
          function recomputeInView() {
            ...
            if (nowIn !== footer.inView)
              footer.inView = nowIn
          }
          function checkGibMeet() {
            if (!footer.heartGibbed || footer.creditHidden)
              return
          ...
          Timer {
            ...
            interval: 1300
            repeat: false
            onTriggered: if (!footer.inView && footer.n === 0)
              footer.cursorPresent = false
          }
          SequentialAnimation {
            id: beatAnim
          ...
          Item {
            ...
            Row {
              ...
              Item {
                implicitWidth: fGh.implicitWidth
                implicitHeight: fGh.implicitHeight
                anchors.verticalCenter: parent.verticalCenter
                Text {
                  id: fGh
  ...
  Rectangle {
    ...
    radius: 10
    color: page.surfaceHi
    border.color: page.outline
    border.width: 1
    implicitWidth: toastRow.width + 32
    implicitHeight: 44
  ...
  Canvas {
    ...
    function releaseWithText(creditItem) {
      ...
      fallText.y = p.y
      fallText.vy = 8
      fallText.active = true
      fallText.visible = true
    ...
    function reset() {
      ...
      running = false
      releasing = false
      fallText.active = false
      fallText.visible = false
      requestPaint()
    }
    ...
    FrameAnimation {
      running: gibOverlay.running
      onTriggered: {
        gibOverlay.step(Math.min(frameTime, 0.032))
        gibOverlay.requestPaint()
      }
    }
    ...
    function step(dt) {
      ...
      if (gibs.length === 0 && drips.length === 0 && !fallText.active && (releasing || bleedTime > 15))
        running = false
    }
    onPaint: {
      var ctx = getContext('2d')
      ...
      for (var i = 0; i < splats.length; i++) {
        ...
        ctx.fillRect(Math.round(s.x), Math.round(groundY - s.size), s.size, s.size)
      }
      ...
      for (i = 0; i < drips.length; i++) {
        ...
        ctx.fillRect(Math.round(d.x), Math.round(d.y), 2, 4)
      }
    }
  }
}
