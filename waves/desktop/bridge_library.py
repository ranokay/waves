...
``waves.library.index`` walks the disk and enumerates album facts;
``waves.metadata.matching`` decides whether two records name the same album; this mixin
...
from waves.paths import folder_name_candidates, safe_filename_replacement, safe_filename_replacement_map
...
def _closed(lib) -> bool:
    ...
    ``is_closed`` is a method, so a bare attribute test on it would be
    always true; it is read here through ``callable`` so that a future
    property spelling answers correctly too, rather than silently reading
    False for every cache."""
...
def _fold_atmos_subfolders(albums: list, by_folder: dict, fragments: set) -> tuple[dict, set]:
    """Fold Atmos subfolders back into their parent albums (§8.4).
...
def _folded_parent_counts(album_id: str, by_folder: dict, sub_tracks: list) -> tuple[int, bool, int | None]:
    """A folded parent's ``(extra_tracks, has_atmos, extra_runtime)`` (§8.4).
...
class LibraryMixin:
    ...
    def _publish_index(self, idx: dict, tracks: dict, *, only_if_unset: bool = False) -> bool:
        ...
        One swap, because deriving the rollup after the album index is
        published leaves a gap that the slots reading both cover with a lazy
        derive: a full pass over the album index, on the GUI thread, inside
        whatever frame happens to ask first. At launch that is a card being
        created behind the boot water (52-84 ms in build_artist_rollup under
        _library_probe_wanted, mid-fade). With the rollup derived HERE, before
        the lock, a reader can never see an index without its rollup, and no
        lazy derive exists.
    ...
    def _rebuild_library_index(self, force_full: bool = False) -> None:  # noqa: C901 (see below)
        ...
        def on_progress(event: dict) -> None:
            ...
            if p["phase"] == "read":
                ...
                # Mid-scan republish, throttled: a full table read per
                # committed flush would rebuild BOTH presence indexes, and a
                # cold scan of a big library commits every 200 albums, so the
                # rebuild cost grows quadratically as the table fills
        ...
        def work() -> None:
            ...
            try:
                ...
                #
                # Inside the finally, and caught: outside it, one raise would
                # skip the coalescing tail below and silently drop a Rescan the
                # user had already pressed, while leaving the pending flag set
                # to fire a spurious full rebuild later.
                try:
                    if gen == self._library_gen:
    ...
    # TIDAL page is missing. The arbiter (waves.metadata.mb_arbiter) may only ever
    ...
    def _library_track_claim(
        ...
        Presence alone is not enough: a title and artist match every
        compilation, best-of and re-release that share them, so "you own this
        song somewhere" would skip tracks out of an album the user explicitly
        asked for, and the folder would land short with nothing on screen
        saying which tracks are missing. The album this track belongs to is
        what makes the question answerable, so it is passed in and the proven
        axis is required.
    ...
    def _library_recover_untrusted(self, lib, root: str, alive, on_progress=None) -> int:
        """(POOL THREAD, the in-process fallback) waves.library.recover's
    ...
    def _library_scan_once(self, lib, root: str, force_full: bool, alive, on_progress) -> tuple[int, str]:
        ...
        holds this process's interpreter (see waves.library.worker).
