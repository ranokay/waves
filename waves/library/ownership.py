...
# LOW < HIGH < LOSSLESS < HI_RES_LOSSLESS, ranked 0..3 by the
...
def record_names_a_broken_copy(rec: dict | None) -> bool:
    """True when the recorded path carries a spelling the name formatter
    never produces: a "[None]" where the release year belonged (what an album
    TIDAL lists no date for renders), or a literal unrendered
    "{album_track_num}" token (the album-404 fallback). Such a copy must not
    satisfy the ownership gate: the formatter can never rebuild those
    spellings, so the gate would freeze the garbage file as the owned copy and
    skip the corrected re-download forever. The stale file itself is left
    alone (the app never deletes user-visible files); the fresh download lands
    at the corrected path and takes over the record."""
...
_ADDED_COLUMNS = (
    ...
    ("audio_mode", "TEXT"),
    # Explicit audio type (§5.3): "stereo" / "atmos" in the seam's
    ...
    # one case where a better master is provably there for the asking, so the
    # upgrade deliberately stays open. But TIDAL can advertise
...
_OWNERSHIP_COLUMNS = (
    ...
    " sample_rate, codecs, recorded_at, requested_rank, ceiling_rank, degraded_tries, atmos_only"
)
...
def _best_surviving(rows, roots: list[str] | None = None) -> dict | None:
    ...
    and the batch answer; the row shape never has to be inferred from its
    width."""
    for row in rows:
        path, tier, rank, mode, atype = row[:5]
        depth, rate, codecs, recorded_at, requested, ceiling, degraded, atmos_only = row[-8:]
        if not path:
            continue
...
class OwnershipStore:
    ...
    def __init__(self, db_path: str) -> None:
        ...
        # them read while one writes). Sharing one connection under ``_lock``
        # instead makes a card asking members_of on the GUI thread wait for
        # whichever refresh worker holds the lock for its own query; that wait
        # is most of the time the cards' creation spends blocked, with the
        # launch water dropping frames on it. An in-memory database cannot be
        ...
        with self._lock:
            ...
            # Integrity skip-list (spec §6.3): provider-scoped
            ...
            self._conn.execute(
                """INSERT INTO downloads
                   ...
                   ON CONFLICT(track_id, path) DO UPDATE SET
                       ...
                       audio_mode   = COALESCE(excluded.audio_mode, downloads.audio_mode),
                       audio_type   = COALESCE(excluded.audio_type, downloads.audio_type),
    ...
    def ownership_of_many(self, track_ids, *, audio_type: str | None = None) -> dict[str, dict | None]:
        ...
        Version, the same filter ``ownership_of`` accepts. The batch query
        selects the same columns as the single one, so a type-only row never
        reads as unknown through this path.
