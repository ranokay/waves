...
caller feeds those facts to ``waves.metadata.matching``, which is where every "is this
...
#
# "Waves Quarantine" is the integrity gate's folder (spec §6.3): a
...
# the integrity gate's custom quarantine location. The default
...
# count cannot come from the raw file count, which is only the album's track
# count when every file in the folder belongs to that album. A "Singles",
# "Inbox" or iTunes-dump folder breaks that badly: 30 unrelated tracks whose
# first file happens to be tagged "Discovery" index as a 30-track Discovery,
# which then satisfies every completeness test and claims a full copy of an
# album the user owned one song of. Sampling a few files per folder would keep
# a cold NAS scan cheap, but the track-level presence pill needs the TITLE of
# every file anyway, so every file is read once, and the same reads double as a
# full-folder agreement check. On disagreement the folder is still indexed
# (its identity and quality are real) but with a track count of 0, which
# makes the matcher decline to claim it rather than claim it wrongly.
...
# §8.4 of the provider spec: which Version a file is, read the way
...
def _default_item_id(path: str) -> str | None:
    ...
    (waves.metadata.tags), so a file answers here exactly as it does there: the
    ...
    """
    try:
        from waves.metadata.tags import read_item_id_or_none
    except Exception:
...
# Anything else is a bug in the caller, not a third view.
FILES_VIEWS = ("saved", "all")

# The Saved view's predicate, in one place: a file Waves itself saved carries
# an item id (NULL is a row from before the column, '' an untagged file the
# walk sees).
...
class LibraryIndex:
    ...
    def __init__(
        ...
        *,
        ...
        # Which Version one file is (§8.4). A separate probe from
        ...
        # Which Waves item one file was saved as (ADR 0007): the
        ...
        with self._lock:
            ...
            for col in (
                ...
                # exists to cure.
                "raw_count INTEGER",
                # §8.4: whether the folder holds Atmos Versions
            ...
            # ADR 0007: the Waves item id one file was saved as.
    ...
    def probe_folders(
        ...
        *,
        ...
        try:
            ...
            # Adding each to the other's total makes the emitted count fall
            # between hits, and a bar that goes backwards reads as a hang, which
            # is the exact symptom this progress feed exists to cure.
            seen_albums = 0
            seen_folders = 0
            live_albums = 0
            live_folders = 0
        ...
        try:
            ...
            while outstanding:
                ...
                if path in verify:
                    ...
                    # listing described: condemned, pruned this scan, instead of
                    # being taken on the listing's word. Anything
    ...
    def _load_album_mtimes(self) -> dict[str, tuple[float, bool]]:
        ...
        for path, mtime, codec, count, recorded, tracks, declared, runtime, has_atmos, unknown, unprobed in rows:
            ...
            # unprobed counts the same shape for item ids: NULL is a row from
            # before item-id capture (or a failed probe), both of which owe
            # the folder one re-read.
            fresh = (
                codec is not None
    ...
    def _track_row(dirpath: str, tags: dict, audio_type: str | None = "stereo", item_id: str | None = "") -> tuple:
        ...
        ``audio_type`` is which Version the file is (§8.4):
        ...
        ``item_id`` is which Waves item the file was saved as (ADR 0007):
        None when the probe failed, persisted as NULL so the folder
    ...
    def _read_row(self, job: _Candidate, alive: Callable[[], bool]) -> tuple | None:
        ...
        §8.4: an Atmos Version is never a second canonical entry. Every
        ...
        # "TOO" needs proven stereo beside proven Atmos: an unclassified file
        # (None: a transient probe failure, retried next scan) reads canonical
    ...
    # rows shaped exactly as the app's in-memory presence indexes are
    ...
    def _album_fact(row) -> dict:
        ...
        return {
            ...
            # Atmos presence travels with the facts: the matcher's
    ...
    def track_facts(self, key) -> list[dict]:
        ...
        holding folder's identity and carrying its featuring credit, the
        shape the matcher is built against."""
        try:
            title_key, artist_key = key
        except (TypeError, ValueError):
            return []
    ...
    def iter_tracks(self) -> Iterator[dict]:
        ...
        for (
            ...
            yield {
                ...
                "length": int(length or 0),
                # Which Version the file is (§8.4): "stereo" /
    ...
    def _unchanged_verdict(row: tuple | None, mtime: float, count: int) -> bool:
        ...
        return (
            ...
            # NULL has_atmos is a row from before Atmos capture (§8.4); one
            # re-read backfills it (0/1, like runtime above).
            ...
            and row[9] == 0
            # NULL item_id rows are from before item-id capture (ADR 0007) or
    ...
    def adopt_scan_outcome(self, status: str, *, partial: bool, shape, reconciled: bool) -> None:
        ...
        file (waves.library.worker): exactly the attributes refresh() would
