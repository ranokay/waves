...
def disc_group(folder: str) -> tuple[str, str] | None:
    ...
    THE CARE REQUIRED: those halves have to be added up, and adding up the
...
def decide_presence(title, artist, year, tracks, index, duration=0) -> dict:
    ...
    return {
        ...
        # canonical set (§8.4): the album card's ATMOS TOO
...
def decide_track_presence(title, artist, index, album="", album_year="", duration=0) -> dict:
    ...
    second. Letting length prove alone lets a copy filed under one
    album answer for another: the pill goes green on a compilation
    whose tracks are nowhere on disk, and the bulk claim gate, which rides
    this axis, skips them out of an album the user had explicitly asked for.
