...
def _tidy_spacing(value: str) -> str:
    ...
    surrounded them, so an album called ``The Better Life / Dead Love`` comes
    out as ``The Better Life  Dead Love``, with a double space where the
    slash was. Runs of whitespace collapse to
...
def folder_name_candidates(
    ...
    For the library scan's probe by name (waves.library.index.probe_folders):
...
def _format_provider(
    ...
    """Handle the provider-separation token.
...
def _fit_name_within_path(directory: pathlib.Path, name: str, sanitize) -> pathlib.Path:
    ...
    album shares, and a respelled folder orphans the album. The extension is
    kept whatever happens, or the file stops reading as
    ...
    The measurement must be in the units the platform itself uses, not a mix
    of two: subtracting a UTF-16 unit count from a UTF-8 byte count agrees for
    ASCII and for nothing else. A CJK title (one unit, three bytes per
    character) would give back a third of the characters it owes, the re-check
    would fail anyway, and the halving backstop would take half the stem — the
    exact over-trim the measured path exists to prevent.
...
def name_comparison_key(value: str) -> str:
    ...
    way and look it up another, which loses a folder.
...
def file_unique_suffix(
    ...
    *,
    ...
    (a stem near the 255-character cap probed untrimmed can hand back a trimmed
    ...
    Args:
        ...
        is_own_copy (Callable[[pathlib.Path], bool] | None, optional): Whether the file
            ...
            replace. Only asked when check_disk is False; without it, "its own older
            copy" is taken on faith, and a DIFFERENT track that happens to share the
            name is replaced too, losing that song. Defaults to
            None, which answers yes to everything.

    Returns:
        str | None: The unique suffix, an empty string if not needed, or None when the
            name and every one of its numbered variants is occupied. Returning the
            last candidate tried would hand back a name that is itself taken, so the
            caller would move onto an occupied name and the finished download would be
            refused there with a collision error that names the wrong cause.
    """
