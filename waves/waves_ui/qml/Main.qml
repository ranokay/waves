...
ApplicationWindow {
  ...
  // the scene (above every page and popup) and nowhere else. They used to
  // be read by a Python event filter on the window, which then saw EVERY
  // event the window receives, the per-frame update request included, and
  // each of those crossings waited for the interpreter behind whatever
  // worker held it: sampled live at launch, that wait was the single
  // largest GUI-thread cost while the landing built (the boot water
  // dropped frames on it). Only these two buttons are accepted, so every
  ...
  component CardCaption: Item {
    ...
    readonly property bool fresh: card.kind === "album" ? root.isNewRelease(card.listed || card.date) : card.kind === "track" ? root.isNewRelease(card.date) : false
  ...
  // (keep-alive), but the tab switch also started a fresh section-scoped
  // trail, so the crumb row held a single, unclickable pill and the only
  ...
  //
  // Cutting on POSITION alone is not enough, and livetesting showed exactly
  // why: position reports a seek target the instant it is set, while the
  // sink still holds whatever frame it had before (for a stream that just
  // started, frame one). The swap then flashed the first frame for a beat
  ...
  // answers lands, every card whose members are in it used to re-ask the
  // bridge for its own rollup, one QML-to-Python call per card (~130 on a
  // fresh landing, each waiting its turn for the interpreter behind the
  // launch workers, sampled 2026-09-12 as a run of 50-80 ms GUI-thread
  ...
  component QualTag: Row {
    ...
    Rectangle {
      visible: qt.mixed
      radius: 4
      color: qt.chromeless ? "transparent" : root.surface2
      border.color: qt.chromeless ? "transparent" : root.outline
      border.width: 1
      implicitHeight: 22
        ...
        Row {
          ...
          Repeater {
            ...
            delegate: Rectangle {
              ...
              height: 6
              radius: 3
              color: root.qualDot(modelData.q)
            }
          }
        }
        Text {
        ...
        Repeater {
          ...
          delegate: Row {
            ...
            Text {
              ...
              color: root.qualSpecFg(modelData.q)
              font.family: root.mono
              font.pixelSize: 9
            }
          }
  ...
  component QualPick: Item {
    ...
    Behavior on extraW {
      NumberAnimation {
        duration: root.hoverMotion ? 220 : 0
    ...
    onOverrideChanged: if (armed && root.hoverMotion) {
      swapping = true
      swapDone.restart()
    }
    onFaceChanged: {
      var prev = lastFace
      lastFace = face
      if (armed && root.hoverMotion && prev !== "")
        startDip(prev)
    }
    ...
    Loader {
      ...
      sourceComponent: ExpandChevron {
        ...
        Behavior on opacity {
          NumberAnimation {
            duration: root.hoverMotion ? 140 : 0
          }
        }
      }
    }
        ...
        enter: Transition {
          ParallelAnimation {
            NumberAnimation {
              ...
              from: 0
              to: 1
              duration: root.hoverMotion ? 120 : 0
            }
            NumberAnimation {
              property: "scale"
              from: 0.8
              to: 1
              duration: root.hoverMotion ? 260 : 0
              easing.type: Easing.OutBack
            }
          }
        }
        exit: Transition {
          ParallelAnimation {
            NumberAnimation {
              ...
              from: 1
              to: 0
              duration: root.hoverMotion ? 110 : 0
            }
            NumberAnimation {
              property: "scale"
              from: 1
  ...
  component QualPickRow: Rectangle {
    ...
    Row {
      ...
      Rectangle {
        ...
        height: 6
        radius: 3
        color: root.qualDot(qpr.t)
      }
      Text {
        textFormat: Text.PlainText
        anchors.verticalCenter: parent.verticalCenter
        text: qpr.t
        color: qpr.lit ? root.qualFg(qpr.t) : root.textHi
        font.family: root.mono
        font.pixelSize: 10
        font.bold: true
        Behavior on color {
      ...
      Text {
        textFormat: Text.PlainText
        anchors.verticalCenter: parent.verticalCenter
        text: root.qualSpec(qpr.t)
        color: qpr.lit ? root.qualSpecFg(qpr.t) : root.textHi
        font.family: root.mono
        font.pixelSize: 10
        Behavior on color {
          ColorAnimation {
    ...
    Row {
      ...
      Text {
        ...
        color: qpr.lit ? root.textHi : root.textLo
        font.family: root.mono
        font.pixelSize: 9
      }
      Text {
  ...
  component LibraryTag: Rectangle {
    ...
    radius: 4
    color: root.pillClassCont(pxt.qclass)
    border.color: root.pillClassDim(pxt.qclass)
    border.width: 1
    implicitHeight: 18
    ...
    Text {
      ...
      color: root.pillClassTx(pxt.qclass)
      font.family: root.mono
      font.pixelSize: 9
      font.bold: true
    }
  ...
  // it is the only way the user finds it (issue #38).
  ...
  component DownIcon: Rectangle {
    ...
    function openLibraryClaim() {
      root.openLibraryClaim(di.mediaId, di.libTitle, di.libPath, di.libAlbum ? "album" : "track")
    }
    ...
    Connections {
      ...
      function onOwnershipChangedBatch(batch) {
        if (root.ownBatchHits(batch, di._ownKeys))
          di.refreshOwned()
      }
    ...
    radius: root.btnRad
    clip: true
    color: libGuess ? root.goldCont : libPartialClaim ? root.cyanCont : st === "done" ? root.greenCont : st === "failed" ? root.redCont : root.accentCont
    border.width: root.btnBorderW
    border.color: libGuess ? root.goldDim : libPartialClaim ? root.cyanDim : st === "failed" ? root.red : st === "done" ? root.greenDim : root.accentDim
    scale: 1
    Behavior on scale {
      NumberAnimation {
        duration: 130
    ...
    Item {
      ...
      anchors.fill: parent
      anchors.margins: root.btnBorderW   // sit inside the border
      ...
      Repeater {
        ...
        delegate: Rectangle {
          ...
          opacity: (di.pct >= 99.9 && litCell) ? 0.62 + 0.38 * (0.5 + 0.5 * Math.cos(2 * Math.PI * (root.shimmerPhase * 2 + twinkleR))) : pulsing ? root.ledPulse : (litCell ? 1.0 : 0.16)
        }
      }
    }
    Item {
      id: diGridMask
      anchors.fill: parent
      anchors.margins: root.btnBorderW
      ...
      Rectangle {
        anchors.fill: parent
        radius: root.btnRad - root.btnBorderW
        color: "#ffffff"
      }
    }
    Text {
    ...
    Ico {
      ...
      color: di.libGuess ? root.gold : di.libPartialClaim ? root.cyan : di.st === "done" ? root.green : root.accent
      size: 15
  ...
  // a ghost marker + time readout, click or drag seeks (seek-lab ghost
  // cursor + segment-lab solid arc). Only the active track shows its ring,
  // one shared player.
  component PreviewArt: Item {
    ...
    // a mix page). Held OFF while this disc's preview is active: the
    ...
    readonly property bool fxOn: root.artFxVariant !== "none" && !pa.active
    property bool fxArmed: false
    function _fxAim(p) {
      var cx = width / 2, cy = height / 2
      fxRy = ((p.x - cx) / cx) * root.artFxTilt
      fxRx = -((p.y - cy) / cy) * root.artFxTilt
    }
    Timer {
      id: paFxArm
      interval: 90
    ...
    // Playback ring: one continuous round-cap arc over a dim track
    // (segment-lab round 2 winner, "solid arc"), drawn with the same
    // CurveRenderer as the Ico glyphs so the curve is genuinely smooth,
    // no rotated-rectangle corners. Fills clockwise from 12 o'clock.
    ...
    Item {
      ...
      Rectangle {
        ...
        height: 6
        radius: 3
        color: played ? root.bg : root.accentSoft
    ...
    Rectangle {
      ...
      Text {
        ...
        anchors.centerIn: parent
        textFormat: Text.PlainText
        text: root.fmtMs(pa.aimFrac * root.previewDuration)
    ...
    MouseArea {
      ...
      onPositionChanged: function (m) {
        ...
        if (ringHover || scrubbing) {
          ...
          if (scrubbing)
            root.scrubPreviewVisual(pa.aimFrac)
        }
      }
      ...
      onReleased: function (m) {
        if (scrubbing) {
          ...
          root.seekPreview(pa.aimFrac)
        } else if (dist(m.x, m.y) <= (pa.active ? discR : idleR)) {
          root.togglePreview(pa.kind, pa.pid, 0)
        }
      }
      onCanceled: {
        scrubbing = false
        ...
        Row {
          ...
          Ico {
            ...
            MouseArea {
              ...
              onClicked: function (m) {
                m.accepted = true
                root.togglePreview(tp.kind, tp.pid, 0)
              }
            }
          }
          Ico {
          ...
          Item {
            ...
            DotMatrix {
              ...
              gap: 2
              pct: tp.pct
              ledPulse: root.ledPulse
              shimmerPhase: root.shimmerPhase
              queueEdgeHeld: root.queueEdgeHeld
            }
            MouseArea {
              anchors.fill: parent
              enabled: tp.live
    ...
    Item {
      ...
      Ico {
        ...
        MouseArea {
          ...
          onClicked: function (m) {
            m.accepted = true
            root.togglePreview(pbar.kind, pbar.pid, 0)
          }
        }
      }
  ...
  component LibPlaylistRow: Rectangle {
    ...
    MouseArea {
      ...
      z: -1
      cursorShape: Qt.PointingHandCursor
      onClicked: plRow.isFolder ? (plRow.folderHost ? plRow.folderHost.openFolder(plRow.model.id, plRow.model.title) : undefined) : root.openPlaylistPage(plRow.model.id, plRow.model.title, plRow.model.art)
    }
  }
  ...
  component SpecBtn: Rectangle {
    ...
    readonly property color bg: danger ? root.redCont : warn ? root.goldCont : (primary ? root.accentCont : "transparent")
    ...
    border.width: 1
    border.color: danger ? Qt.alpha(root.red, 0.55) : warn ? Qt.alpha(root.gold, 0.55) : (primary ? root.accentDim : root.border1)
    Behavior on color {
      ColorAnimation {
        duration: 110
      }
    ...
    Text {
      ...
      textFormat: Text.PlainText
      text: sb.label
      color: sb.danger ? root.red : sb.warn ? root.gold : (sb.primary ? root.accent : (sbMa.containsMouse ? root.textHi : root.textLo))
  ...
  component GateCard: Rectangle {
    ...
    radius: 10
    color: highlight ? (gcMa.containsMouse ? Qt.lighter(root.accentCont, 1.35) : root.accentCont) : (gcMa.containsMouse ? root.surface3 : root.surface0)
    border.width: 1
    border.color: highlight ? root.accentDim : (gcMa.containsMouse ? root.outline : root.border1)
    RowLayout {
      id: gcRow
      anchors.verticalCenter: parent.verticalCenter
      x: 14
      ...
      ColumnLayout {
        ...
        RowLayout {
          ...
          Text {
            textFormat: Text.PlainText
            text: gcard.title
            color: gcard.highlight ? root.accent : root.textHi
            font.pixelSize: 13
            font.bold: true
          }
          Text {
      ...
      Ico {
        name: "arrow-right"
        color: gcard.highlight ? root.accent : root.textDim
        size: 15
  ...
  component SectionHeader: Item {
    ...
    RowLayout {
      ...
      Text {
        ...
        color: (secHead.collapsible && secMa.containsMouse) || (secHead.openable && secOpenMa.containsMouse) ? root.textHi : root.textLo
        font.pixelSize: 12
        font.bold: true
    ...
    MouseArea {
      ...
      onClicked: {
        ...
        if (collapsing && sa.sectionTop)
          root.scrollCollapsedToSection(sa.sectionTop)
      }
    }
  }
  ...
  component SearchProviderGroup: Column {
    ...
    function sectionVisible(name) {
      return !group.collapsed && root.sectionVisible(name, group.countFor(name))
    }
    ...
    readonly property bool headVisible: {
      if (group.providerId === "")
        return false
      if (root.filterType !== "all" && !group.hostable(root.filterType))
        return false
    ...
    Item {
      ...
      Rectangle {
        ...
        anchors.bottom: parent.bottom
        height: 2
        color: group.accentHead ? root.accentDim : root.outline
      }
      MouseArea {
        id: groupHeadMa
        anchors.fill: parent
    ...
    Flow {
      ...
      Repeater {
        ...
        delegate: Loader {
          ...
          visible: group.stripArtists ? true : root.searchRowVisible("artists", artistsModel.count, index, group.isExpanded("artists"))
    ...
    ShowAllLabel {
      ...
      visible: group.sectionVisible("artists") && root.filterType === "all" && artistsModel.count > 0 && (group.isExpanded("artists") || (group.stripArtists ? artistStrip.contentWidth > artistStrip.width + 1 : artistsModel.count > 5))
  ...
  component VideoThumb: Item {
    ...
    // card out of its rect (root.peekOpen). Thumbs without an id (none
    // today) stay inert.
    ...
    Timer {
      ...
      onTriggered: if (vtHover.hovered)
        root.peekOpen(vt, vt.videoId, vt.vTitle, vt.vArtist, vt.url)
    }
  }
  ...
  component StyledCombo: ComboBox {
    ...
    background: Rectangle {
      radius: 8
      color: root.surface2
      border.color: sc.popup.visible ? root.accent : root.outline
    }
    contentItem: Text {
      textFormat: Text.PlainText
      text: sc.displayText
  ...
  component AlbumBlock: Column {
    ...
    readonly property var leadArtists: {
      var a = root.artistsById[albumId] || []
      if (a.length > 0)
        return a
      return artistName !== "" ? [
        {
    ...
    function toggle() {
      ...
      if (e[albumId])
        Qt.callLater(function () {
          if (ab.expanded)
            root.rememberExpandReturn(key, root.scrollExpandedIntoView(ab))
        })
      else
        root.scrollCollapsedBack(key, ab)
    }
    ...
    Rectangle {
      ...
      RowLayout {
        ...
        Text {
          ...
          color: expanded ? root.accent : root.textDim
          font.pixelSize: 16
        ...
        ColumnLayout {
          ...
          Text {
            ...
            MouseArea {
              ...
              onContainsMouseChanged: containsMouse ? root.hoverPrefetch(prefetchCard) : root.hoverPrefetchCancel(prefetchCard)
            }
            AlbumPresencePill {
              id: abRowPill
              anchors.verticalCenter: parent.verticalCenter
      ...
      MouseArea {
        ...
        onContainsMouseChanged: containsMouse && !ab.expanded ? root.hoverPrefetch(prefetchCard) : root.hoverPrefetchCancel(prefetchCard)
      }
    }
        ...
        Column {
          ...
          RowLayout {
            ...
            Rectangle {
              ...
              border.color: root.accentDim
              border.width: 1
              implicitHeight: dsRow.implicitHeight + root.btnPadV * 2
              implicitWidth: dsRow.implicitWidth + root.btnPadH * 2
              Row {
                id: dsRow
                anchors.centerIn: parent
                spacing: 7
  ...
  component PlaylistBlock: Column {
    ...
    function toggle() {
      ...
      if (e[plId])
        Qt.callLater(function () {
          if (pb.expanded)
            root.rememberExpandReturn(key, root.scrollExpandedIntoView(pb))
        })
      else
        root.scrollCollapsedBack(key, pb)
    }
        ...
        Column {
          ...
          RowLayout {
            ...
            Rectangle {
              ...
              border.color: root.accentDim
              border.width: 1
              implicitHeight: pbDsRow.implicitHeight + root.btnPadV * 2
              implicitWidth: pbDsRow.implicitWidth + root.btnPadH * 2
              Row {
                id: pbDsRow
                anchors.centerIn: parent
                spacing: 7
  ...
  component NewTag: Item {
    ...
    Rectangle {
      ...
      readonly property bool breathing: visible && root.onScreen && root.bootContentShown >= 1 && !nt.settled
      function breathe() {
        ntPulse.stop()
        ntBeat.stop()
        opacity = 1
        if (!breathing)
          return
        ntBeat.interval = Math.max(1, root.newPulseMs - Date.now() % root.newPulseMs)
        ntBeat.start()
      }
  ...
  component TrackRow: Rectangle {
    ...
    HoverHandler {
      ...
      onHoveredChanged: hovered ? root.hoverPrefetch(trow.prefetchCard, 450) : root.hoverPrefetchCancel(trow.prefetchCard)
    }
    // Framed card row:
    ...
    Rectangle {
      ...
      MouseArea {
        ...
        anchors.fill: parent
        hoverEnabled: true
        readonly property bool linkable: trow.kind === "video" || (trow.albumId !== "" && !root.onAlbumPage(trow.albumId))
        ...
        onClicked: trow.revealable ? waves.revealLibraryAlbum(trow.folderPath) : trow.kind === "video" ? root.openVideo(trow.tId, trow.title, trow.artistName) : root.openAlbumPage(trow.albumId, trow.tId, trow.album, trow.art)
      }
        ...
        ColumnLayout {
          ...
          Text {
            ...
            MouseArea {
              ...
              hoverEnabled: enabled
              cursorShape: enabled ? Qt.PointingHandCursor : Qt.ArrowCursor
              onClicked: trow.revealable ? waves.revealLibraryAlbum(trow.folderPath) : trow.kind === "video" ? root.openVideo(trow.tId, trow.title, trow.artistName) : root.openAlbumPage(albumId, tId, trow.album, trow.art)
            }
  ...
  component BrowseCard: Rectangle {
    ...
    function resolveLibPresence(live) {
      ...
      libPresence = (!live && ("lib" in c) && c.libStamp === root.libStamp) ? c.lib : waves.libraryAlbumPresence("" + (c.artist || ""), "" + c.title, "" + (c.year || ""), c.tracks || 0, c.duration_sec || 0)
    }
    ...
    readonly property color libInk: bc.libState === "proven" ? root.green : bc.libState === "maybe" ? root.gold : bc.libState === "partial" ? root.cyan : root.accent
    Column {
      anchors.left: parent.left
      anchors.right: parent.right
      anchors.top: parent.top
      ...
      Item {
        ...
        Row {
          ...
          Text {
            ...
            visible: !bcPv.tight
            textFormat: Text.PlainText
            text: bc.pvSt === "" ? "PREVIEW" : bc.pvSt === "loading" ? "[buffering]" : bc.pvSt === "error" ? "RETRY" : root.fmtMs(root.previewPosition)
            color: bc.pvSt === "error" ? root.red : root.accent
      ...
      Item {
        ...
        MouseArea {
          ...
          onClicked: {
            ...
            if (bc.libClaim) {
              root.openLibraryClaim(bc.card.id || "", "" + (bc.card.title || ""), "" + (bc.libPresence.local_album_id || ""), "album")
              return
            }
          ...
          Row {
            ...
            Item {
              ...
              MouseArea {
                ...
                // a tail slice of (root.crumbBase). A synthetic
                // section-root pill (root.crumbSynth) leads the
                ...
                onClicked: {
                  if (root.crumbSynth && nc.ord === 0)
                    root.crumbSynthGo()
                  else
                    root.navTo(root.crumbBase + nc.ord - (root.crumbSynth ? 1 : 0))
                }
              }
            }
          }
  ...
  component ArtCard: Item {
    ...
    function refreshOwned(live) {
      ...
      applyOwn((!live && ("own" in ac.card) && ac.card.ownGen === root.ownGen) ? ac.card.own : waves.collectionOwnership("" + (ac.card.id || "")))
    }
    ...
    Connections {
      ...
      function onOwnAnswersGenChanged() {
        var a = root.ownAnswers["" + (ac.card.id || "")]
        if (a)
          ac.applyOwn(a)
      }
    }
    ...
    readonly property bool haveIt: ac.owned || ac.libFull || root.dlSt(ac.card.id || "") === "done"
    ...
    Art {
      ...
      // hero card, and it sat flat while every card around it moved
      ...
      readonly property bool controlsOn: collection && (acWrapHover.hovered || root.dlSt(ac.card.id || "") !== "" || root.pvSt(ac.kind, ac.card.id || "") !== "")
      ...
      RiseIn {
        ...
        RollSwap {
          ...
          liveBorder: acLive.dlOn ? acDl.edge : (acRiser.acPvSt === "error" ? root.red : root.accentDim)
          ...
          idleItem: Item {
            ...
            readonly property color stInk: dlDone ? root.green : ac.libState === "proven" ? root.green : ac.libState === "maybe" ? root.gold : ac.libState === "partial" ? root.cyan : root.accent
            readonly property color stEdge: dlDone ? root.greenDim : ac.libState === "proven" ? root.greenDim : ac.libState === "maybe" ? root.goldDim : ac.libState === "partial" ? root.cyanDim : root.accentDim
            ...
            readonly property real naturalW: acStripPv.implicitWidth + acStripDl.implicitWidth + root.btnBorderW + 2 * padFull
            ...
            HoverSwell {
              ...
              width: acStrip.dividerX + root.btnBorderW
            ...
            Row {
              ...
              Item {
                ...
                MouseArea {
                  ...
                  onClicked: {
                    if (acStrip.dlDone) {
                      root.openRedownloadGate(ac.card)
                      return
                    }
                    if (ac.libClaim)
                      root.openLibraryClaim(ac.card.id || "", "" + (ac.card.title || ""), "" + (ac.libPresence.local_album_id || ""))
                    else
                      root.browseCardDownload(ac.card)
                  }
                }
      ...
      RiseIn {
        on: !acArt.collection && (acWrapHover.hovered || root.dlSt(ac.card.id || "") !== "")
        anchors.right: parent.right
        anchors.bottom: parent.bottom
        anchors.margins: 10
        width: 38
  ...
  component BrowseScroll: Flickable {
    id: bsp
    ...
    function maybeGrow() {
      if (!grows || root.browsePageKey === "" || !root.browsePage)
        return
      var secs = root.browsePage.sections || []
      if (secs.length !== 1 || !root.browseCanGrow(secs[0]))
        return
      if (contentY + height >= contentHeight - 800)
        root.browseGrow(secs[0])
    }
  ...
  component BrowseSection: Column {
    ...
    readonly property bool grid: sec.rowKind === "cards" && !landing && !(root.browsePage && root.browsePage.header) && ((root.browsePage && root.browsePage.sections) || []).length === 1
    ...
    function openListing() {
      if (sec.more)
        root.openBrowseLink(sec.more, sec.title || "More")
      else
        root.openBrowseSection(sec)
    }
    spacing: 8
    SectionHeader {
    ...
    Item {
      ...
      width: parent.width
      height: 46
      readonly property bool loadingMore: !!(bsec.sec.data && root.browseGrowing[bsec.sec.data])
      Row {
        anchors.centerIn: parent
        spacing: 10
        visible: parent.loadingMore || !root.browseCanGrow(bsec.sec)
        Rectangle {
          ...
          Component {
            ...
            Rectangle {
              ...
              MouseArea {
                anchors.fill: parent
                cursorShape: Qt.PointingHandCursor
                onClicked: blinkLd.modelData.pl ? root.openPlaylistsFolder(blinkLd.modelData.path, blinkLd.modelData.title) : root.openBrowseLink(blinkLd.modelData.path, blinkLd.modelData.title)
              }
            }
          }
        }
  ...
  component BrowseTile: Rectangle {
    ...
    readonly property var tones: [[root.accentCont, root.accentDim, root.accentContTx], [root.goldCont, root.goldDim, root.goldContTx], ["#0a2126", root.cyanDim, "#9fdbe6"], [root.redCont, "#8a3a34", "#ffb3ad"], [root.greenCont, root.greenDim, root.greenContTx], [root.surface3, root.outline, root.textHi],]
    ...
    property real fxRy: 0
    property real fxLift: 1.0
    readonly property bool fxOn: root.artFxVariant !== "none"
    ...
    MouseArea {
      ...
      hoverEnabled: true
      cursorShape: Qt.PointingHandCursor
      onClicked: bt.plOnly ? root.openPlaylistsFolder(bt.path, bt.title) : root.openBrowseLink(bt.path, bt.title)
    }
    ...
    RiseIn {
      ...
      RollSwap {
        ...
        idleItem: Item {
          ...
          HoverSwell {
            z: 1    // above the Row, so the lit edge covers the divider
            width: catStrip.dividerX + root.btnBorderW
  ...
  component LibChip: Rectangle {
    ...
    Row {
      ...
      Rectangle {
        ...
        radius: 3
        anchors.verticalCenter: parent.verticalCenter
        color: chip.on ? root.accent : root.textDim
      }
      Text {
        textFormat: Text.PlainText
        anchors.verticalCenter: parent.verticalCenter
        text: chip.label
        color: chip.on ? root.accent : root.textLo
        font.pixelSize: 13
      }
    }
    MouseArea {
  ...
  component LibSourceGroup: ColumnLayout {
    ...
    onCategoryChanged: if (group.primary)
      root.libraryCategory = group.category
    ...
    RowLayout {
      ...
      ComboBox {
        ...
        background: Rectangle {
          radius: 8
          color: root.surface2
          border.color: libSortBox.popup.visible ? root.accent : root.outline
        }
        contentItem: Text {
          textFormat: Text.PlainText
      ...
      GridView {
        ...
        delegate: Item {
          ...
          Rectangle {
            ...
            anchors.margins: 5
            radius: 10
            color: agMa.containsMouse ? root.surface2 : root.surface
            border.color: root.border1
            Column {
              anchors.fill: parent
              anchors.margins: 8
            ...
            Item {
              ...
              Row {
                ...
                Text {
                  textFormat: Text.PlainText
                  text: agPv.pst === "" ? "PREVIEW" : agPv.pst === "loading" ? "[buffering]" : agPv.pst === "error" ? "RETRY" : root.fmtMs(root.previewPosition)
                  color: agPv.pst === "error" ? root.red : root.accent
      ...
      LibList {
        ...
        delegate: Rectangle {
          ...
          MouseArea {
            ...
            cursorShape: Qt.PointingHandCursor
            z: -1
            onClicked: root.openVideo(model.id, model.title, model.artist)
          }
        }
      }
  ...
  ColumnLayout {
    ...
    Rectangle {
      ...
      // Universal seek on the bottom playback line (seek-lab "spark"):
    ...
    ColumnLayout {
      ...
      ListView {
        ...
        section.delegate: Item {
          ...
          RowLayout {
            ...
            Text {
              ...
              color: secItem.section === "completed" ? root.accentContTx : secItem.section === "failed" ? root.red : secItem.section === "stopped" ? root.redContTx : secItem.section === "downloading" ? root.textHi : root.textDim
              font.family: root.mono
              font.pixelSize: 10
              font.bold: true
              font.letterSpacing: 1.4
          ...
          onSectionChanged: {
            secPulse.stop()
            secLbl.scale = 1
            if (secArmedTick === root.pulseTick && secPulseDue())
              secArm.restart()
          }
          SequentialAnimation {
            id: secPulse
        ...
        delegate: Item {
          ...
          readonly property int ledgerShown: qexp ? Math.min(ledgerLen, root.queueLedgerMax) : ledgerHolding ? Math.min(ledgerLen, root.queueLedgerPeek) : 0
          function qtoggle() {
          ...
          readonly property string tier: tierMix.length > 1 ? "" : landed !== "" ? landed : root.tierFloor(model.quality, model.expected)
          width: ListView.view.width
          ...
          Rectangle {
            ...
            radius: 8
            color: qrow.isComp ? root.surface : root.surface2
            border.color: qrow.isComp ? (cardHover.containsMouse && qrow.peekable ? root.outline : root.line1) : qrow.st === "done" ? root.greenDim : cardHover.containsMouse ? root.outline : root.border1
            clip: true
            Behavior on border.color {
              ColorAnimation {
                duration: 120
            ...
            ColumnLayout {
              ...
              RowLayout {
                ...
                Text {
                  ...
                  color: (qrow.qexp || cardHover.containsMouse) ? root.accent : root.textDim
                  font.pixelSize: 13
                  Layout.alignment: Qt.AlignVCenter
                  Layout.leftMargin: -5
                ...
                Rectangle {
                  ...
                  radius: 8
                  color: root.greenCont
                  border.color: root.greenDim
                  implicitHeight: 24
                ...
                RetryMark {
                  ...
                  Layout.alignment: Qt.AlignVCenter
                  visible: root.retryableStatus(qrow.st)
                  color: root.accent
                  box: 16
                  MouseArea {
                    anchors.fill: parent
              ...
              Item {
                ...
                ColumnLayout {
                  ...
                  Text {
                    textFormat: Text.PlainText
                    visible: qrow.ledgerHolding && !qrow.single && (root.queueTracks[qrow.model.qid] || []).length === 0
                    text: "Loading tracks…"
                    color: root.textDim
                    font.family: root.mono
                    font.pixelSize: 10
                  ...
                  Repeater {
                    ...
                    delegate: RowLayout {
                      ...
                      Text {
                        ...
                        // rest stay quiet.
                        color: td.status === "failed" ? root.red : (td.status === "done" || td.status === "skipped" || td.status === "owned") ? root.textHi : root.textLo
                        Behavior on color {
                          ColorAnimation {
                            duration: 260
                          }
                      ...
                      Text {
                        ...
                        readonly property string tier: root.queueTrackTier(delivered, "" + (td.status || ""), "" + (qrow.model.quality || ""), "" + (td.expected || ""))
                      ...
                      Item {
                        ...
                        DecryptText {
                          ...
                          color: td.status === "done" ? root.accent : td.status === "failed" ? root.red : td.status === "unavailable" ? root.redDim : (td.status === "skipped" || td.status === "owned") ? (td.owned === "claim" ? root.libAccent : root.accent) : root.textDim
                          Behavior on color {
                            ColorAnimation {
                              duration: 260
                            }
    ...
    Item {
      ...
      Rectangle {
        ...
        gradient: Gradient {
          ...
          GradientStop {
            position: 0.42
            color: Qt.rgba(root.accent.r, root.accent.g, root.accent.b, 0.93)
          }
          GradientStop {
            position: 0.58
            color: Qt.rgba(root.accent.r, root.accent.g, root.accent.b, 0.93)
      ...
      RowLayout {
        ...
        Repeater {
          ...
          delegate: Rectangle {
            ...
            Text {
              ...
              textFormat: Text.PlainText
              text: modelData[1]
              color: on ? root.accent : root.textLo
              font.pixelSize: 11
              font.bold: true
            }
            MouseArea {
        ...
        Rectangle {
          ...
          Text {
            ...
            textFormat: Text.PlainText
            text: "FOLLOW"
            color: logsDrawer.logsFollow ? root.accent : root.textLo
            font.pixelSize: 11
            font.bold: true
          }
          MouseArea {
  ...
  // ==== Open-water launch sequence ("Z FILL", picked in the boot lab) ====
