...
- **Album & artist as first‑class units**: rich artist pages (bio, discography, EPs & singles, top tracks) with one‑click **save‑the‑whole‑thing** actions. A reissue or anniversary edition sits in the discography at the day it was released, and album pages show the full release date. Albums and songs released in the last two weeks wear a small NEW mark, on cards, search results and every track of an album page, which gently pulses until you have them and then stays lit.
- **Smart whole‑artist saving**: per‑source toggles (albums, EPs & singles, features, compilations, music videos), keeping just the **most complete edition** of each album by default while preserving genuinely different releases like remasters and live takes. An artist page shows the same thing it would save: with that on, an edition whose songs all sit inside a fuller edition of the same album is left off the page, and a switch in Settings ("Show every edition on artist pages") lists them all again when you want the full picture. Features and compilations pull only the tracks the artist actually appears on, never another artist's whole album.
