...
def test_a_standalone_dest_matches_the_audio_relative_path(tmp_path):
    ...
    relative = runner.relative_path(
        stub._apple_job_hooks(), track=track, album=None, playlist=None, file_template=template
    )
...
def test_a_collection_standalone_dest_matches_the_album_relative_path(tmp_path):
    ...
    relative = runner.relative_path(
        stub._apple_job_hooks(), track=track, album=album, playlist=None, file_template=template
    )
...
def test_a_template_failure_writes_one_sanitized_name(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner, "format_apple_path", boom)
    stub = _stub()
...
def test_a_render_that_comes_back_empty_uses_the_sanitized_fallback(tmp_path, monkeypatch):
    monkeypatch.setattr(runner, "format_apple_path", lambda *args, **kwargs: "")
    stub = _stub()
