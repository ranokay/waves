"""Two in-flight twins, the late skip, and the rate-limit pause gate.
...
user-visible sequence it prevents.

  1  the late skip only recognises a twin tagged on the BASE name, so a twin at
     a numbered variant, and a twin whose file carries no id at all, each get a
     byte-identical second copy that nothing ever removes
  2  the loser of two in-flight twins is refused at the move and reported as a
     FAILED track, over a file sitting correctly on disk
  3  two batch pauses in force at once: the first to wake reopens the gate for
     everybody, so the run goes back to full API traffic mid-pause

Three more guards cover the holes around those: the refused move logs a twin's
step-aside in the words of a lost download, the exhausted-name path fails an
item whose own twin holds the last name, and the counted pause binds every
worker except the one that took it.
"""
...
def _run_to_the_guard(dl: Download, dst: pathlib.Path, media) -> tuple[bool, pathlib.Path]:
    ...
    guard let the pass through, which is the distinction every late-skip test
...
# evidence of the strongest kind (this run made the file), so the contract is
# untouched: an UNKNOWN identity on disk is still a stranger, never "mine".
# --------------------------------------------------------------------------- #
...
def test_an_untagged_file_this_run_did_not_write_is_still_a_stranger(tmp_path):
    """The contract: only the run's own record speaks for an unreadable file.
    An untagged occupant the run never wrote is the collision the claim exists
    to step around, and reading it as this item makes a distinct track skip
    instead of uniquifying, sidecars and all."""
    dl = _make_download(tmp_path)
    dst = tmp_path / "Song.flac"
...
def test_the_gate_stays_shut_until_the_last_pause_ends():
    """Worker A is standing back, worker C reaches the next boundary and stands
    back too, A wakes first. Its wake-up must not put the rest of the run back
    on the API while C is still pausing."""
    dl = _paced(2, 30.0)
