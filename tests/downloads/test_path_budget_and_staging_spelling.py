"""Path-length budget and existing-spelling behaviour of the naming engine.

The m3u staging name, an existing library's m3u spelling, {folder_path}
resolution, numbered-copy scans, zero-byte variants, whole-path measurement and
uniquification all have to agree on one budget; each test here pins one of
those agreements.
"""
...
from waves import paths as path_module
...
from waves.providers.tidal_folders import apply_folder_path
...
class TestTheM3uStagingNameIsBudgeted:
    """The m3u temp-and-swap builds its staging name (+42 chars) within the
    same length budget as the playlist name, so a name the sanitizer fitted to
    the cap still opens its staging sibling."""
...
class TestTheM3uKeepsItsOldName:
    """An existing library's m3u keeps the name it has: a respelled second file
    would be ingested twice by a library scanner, and nothing may ever delete
    one."""
...
class TestTheFolderPathKeepsItsOldSpelling:
    """{folder_path} resolves to an existing folder's own spelling before the
    stand-ins or the spacing tidy apply; respelling it would re-download every
    playlist inside it into a second tree."""
...
class TestTheVariantScanSpellsNamesTheWayTheyAreWritten:
    """The numbered-copy scan must spell candidates the way the writer does
    (stem trimmed to the 255-byte cap, then _NN): a scan that concatenates raw
    looks for a name that cannot exist, and every re-run of the album adds
    another full copy."""
...
class TestAZeroByteVariantIsNotEvidence:
    """A 0-byte stem_NN leftover from a crash is not evidence that a colliding
    track is present; the base name already refuses that trap through
    check_file_exists."""
...
class TestTheAnswerIsThePathNotABool:
    """When the item lives at a numbered variant, the caller needs the path,
    not a bool: skipping and symlinking against the BASE name would delete the
    fresh audio and point the playlist entry at the colliding stranger's
    file."""
...
class TestTheWholePathCapIsMeasuredHonestly:
    ...
    260, so Windows paths 3 to 15 characters past the real 259 limit would sail
    through and fail at the final move. The sanitizer measures the whole
    spelling itself."""
...
class TestUniquifyRespectsTheWholePathCap:
    """Inserting _01 into a path fitted at the cap must stay under the limit,
    or the move fails after the download."""
...
class TestStagingSurvivesAParentNearTheCap:
    """A parent within 42 bytes of the cap must still stage: the uuid gives
    ground before the staging attempt overflows."""
