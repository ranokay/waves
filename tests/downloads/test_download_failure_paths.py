"""Failure paths in the segment downloader, the metadata writer and the
pause wait.

Covers:
  * a fully failed single-URL (BTS) track is marked corrupt, not "success";
  * _segments_merge failure on the sole segment is a real failure;
  * Metadata.save() raises a clear, catchable error for unreadable files;
  * the .m3u8 playlist writer emits single '\\n' terminators (no os.linesep);
  * the pause wait is abort/stop aware and does not block forever.
"""
...
from waves.metadata.tags import Metadata, MetadataUnreadable
...
class _FakeTask:
    ...
    `_download_segments` runs its segment pass exactly once, then reads
...
class TestSingleUrlFailureIsCorrupt:
    """The last-URL leniency must not exempt a single-URL BTS track."""
...
class TestSegmentsMerge:
    """_segments_merge must treat a sole-segment failure as real."""
...
class TestMetadataUnreadableGuard:
    """Metadata.save() must raise a clear error, not AttributeError."""
...
class TestWaitWhilePaused:
    """The pause wait must honor abort and per-item stop, never block forever."""
