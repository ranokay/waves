...
* The breadcrumb dump limiter must hold one trail per window across both logger
  trees: one handler on each means an error from a waves.* logger and one from
  a third-party root-tree logger seconds apart each write the whole trail
  inside the window the limiter exists to hold.
* A factory reset must not keep the Windows updater's own update.log, which
  quotes install paths carrying the user's name, or a crashed helper's batch
  file.
* Deep on Windows, the staging name's unique part needs no floor: a floor of
  ten characters makes parents in the last ten characters before the cap fail
  every staging attempt while the destination itself fits.
* The merge handler must not read a loop variable, or a target file that will
  not open at all replaces the real error with a NameError.
* The name trim must count in the units the cap does: subtracting a UTF-16 unit
  count from a UTF-8 byte count agrees for ASCII and nothing else, so a CJK
  title would give back a third of what it owed and the halving backstop would
  then take half the stem.
"""
...
# --------------------------------------------------------------------------- #
# one trail per window, not one per logger tree
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# the reset takes Waves' own files with it
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# a staging name that fits beside a destination that fits
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# the real error, not a NameError over it
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# the trim gives back what it owes, in the units it owes them in
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# the redactor learns a credential when it is minted, not once at login
# --------------------------------------------------------------------------- #
