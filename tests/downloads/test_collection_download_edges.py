"""Four edge behaviours of a collection download.

* A mix is the one collection whose items() hands back tracks and videos
  together: without a filter, full music videos land in the mix folder with the
  "music videos" switch off everywhere.
* The m3u's order fix compares this run's paths to the folder listing with raw
  ...
  misses the folder set, the counts disagree and the order silently stands
  down: the failure lands on the libraries most likely to hold it.
* An empty playlist reaches the outcome tallies as (0, 0, 0), which is also
  what a free account's every-stream-refused run looks like, so the row must not
  go red "Failed: no tracks were downloaded" over a collection that had nothing
  to do. The free-account verdict keeps its own answer.
* A symlinked entry on a Windows mapped drive raises ValueError out of the
  relative-path computation (the drive resolves to its UNC spelling, so the two
  paths have different anchors), which must not fail the whole job at its last
  step with every track already landed, nor leave the hidden temp behind
  through every retry.
"""
...
from waves.desktop.backend import _collection_incomplete_reason
from waves.download import Download
from waves.providers.tidal_client import items_results_all
...
# --------------------------------------------------------------------------- #
# a mix obeys the music-videos switch like every other collection
# --------------------------------------------------------------------------- #
def _mix(items):
    mix = Mix.__new__(Mix)
...
# --------------------------------------------------------------------------- #
# the folder's spelling and this run's are one file
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# nothing to do is not something gone wrong
# --------------------------------------------------------------------------- #
...
def test_every_stream_refused_is_still_a_failure():
    ...
    # And with no count at all (a caller that cannot say), the failure still stands.
...
# --------------------------------------------------------------------------- #
# a symlink whose target will not relate to its own folder
# --------------------------------------------------------------------------- #
