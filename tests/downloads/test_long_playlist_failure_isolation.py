"""A long playlist must not fail as a whole on a per-track hazard.
...
essentially every run. Three such hazards would turn one entry into a red row
over a folder holding hundreds of songs:

* a track TIDAL no longer carries must not count as a FAILURE at the
  preparation gate: a stale entry (which a years-old playlist always has)
  would otherwise fail the whole playlist, permanently, on every retry;
...
from waves.desktop.backend import _collection_incomplete_reason, _TrackedDownload
from waves.download import Download
from waves.errors import DownloadIncomplete
from waves.metadata.naming import get_album_artists
...
# The same rule the stream fetch follows: the preparation gate runs FIRST and
# re-fetches every track of a collection (`session.track(id, with_album=True)`),
# so a delisted track 404s there and never reaches the stream fetch at all.
# Swallowing the 404 into a bare "no media" makes item() report ok=False, which
# the job counts as a failure.
...
def test_a_transient_error_at_the_gate_is_still_a_failure_and_is_logged():
    ...
    invisible: the failure must be logged, not swallowed by a blanket catch."""
    session = MagicMock()
...
def test_a_wait_that_ends_on_the_clock_still_retries():
    """Only a STOP refuses. A wait that simply timed out is an ordinary
    backoff and the ladder carries on."""
    retry = _api_retry(_hardened_session())
    waited: list[float] = []
...
def test_a_tuned_pace_survives_the_migration_running_a_second_time():
    ...
    downgrade and back strips the marker and runs this migration again. The
    migration must leave a pace tuned in between alone, not reset both fields
    outright."""
    from waves.config import _migrate_settings
    from waves.model.cfg import Settings as ModelSettings
...
def test_every_catalog_call_carries_a_timeout():
    """tidalapi passes none, so a black-holed connection would park a download
