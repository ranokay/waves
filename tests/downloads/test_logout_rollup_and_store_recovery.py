"""Behavioural edges around sign-out, rollups, store recovery and error text.

* signing out during a search must not latch the busy spinner
* a member that failed and then landed must not leave its rollup red
* a withdrawn row must release both of the marks it registered, and a stale
  batch must drop its merge plan
* a broken ownership file, an unwritable config and a duplicate-column race
  must not kill the launch
* an OSError's filename is the artist, album and title, and must be redacted
* the unique-suffix budget must measure the path the way the platform does
"""
...
# --------------------------------------------------------------------------- #
# a stale refusal must not leave its stash behind
# --------------------------------------------------------------------------- #
class _EnqueueStub:
...
# --------------------------------------------------------------------------- #
# an OSError carries a filename, and inside a download that filename is
...
# --------------------------------------------------------------------------- #
# a kept backup is a whole copy of the app; say so where it stays said
# --------------------------------------------------------------------------- #
