...
Integrity failures already quarantine the offending copy, but the failed row
only carried the words and there was no way to reach or clear the bytes from
the app. These tests drive the real bridge methods: a kept copy is recorded
against its queue row, the row's count drives the drawer actions, opening
reveals the holding folder, and deleting removes the copies (pruning the
folders they emptied) while leaving the skip-list to REDOWNLOAD.
...
def test_delete_removes_the_copies_and_prunes_the_emptied_folders(tmp_path):
    ...
    assert stub.last_status == "Quarantined copy deleted"
