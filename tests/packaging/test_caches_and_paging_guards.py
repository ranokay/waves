"""Cache and paging guards for the bridge's library surfaces.

Hermetic: real unbound WavesBridge/updater/helper functions bound onto minimal
stubs; no Qt, no network. One test per guard, so each can be
...
# _browse_pages is capped like every sibling cache.
...
# The ownership cache is bounded.
...
class _MediaListsStub:
    ...
    def __init__(self, cached_tree):
        ...
        # The listing sweep rides the Provider seam and is the
...
# Revalidation must not discard the user's endless-scroll growth.
...
# The progress relay survives until queued track events drain.
...
# A token with no value substitutes "", never a literal brace.
...
# {album_artist} with no credits must not IndexError.


def test_album_artist_with_no_credits_is_empty_not_a_crash():
    with patch("waves.metadata.naming.get_album_artists", return_value=[]):
...
# Uniquify bounds the FILENAME, not the whole path.


def test_uniquify_leaves_a_short_name_at_a_deep_path_alone():
    deep = pathlib.Path("/" + "/".join(["d" * 10] * 40)) / "song.flac"
    with patch("waves.paths.file_unique_suffix", return_value="_01"):
