"""Stale re-fetch and settings-refresh guards.

Backend and updater behaviour is exercised hermetically (real unbound methods
bound onto minimal stubs, no Qt, no network); the QML side (Browse
RETRY routing, recycled-row selection reset, UNC folder paths, the settings
refresh hook, the Back-restore latch) is pinned by source guards, the same
...
class _RetryStub:
    ...
    def __init__(self, session_track=None):
        ...
        self.statuses: list = []
        self.downloads: list = []
        # The retry refetch resolves through the provider: the
    ...
    def _download(self, obj, typ, name, template, collection, media_id, merge_plan=None, keep_ask=None, **kwargs):
...
# An inherited $APPIMAGE must not make the updater clobber a foreign file.
...
# Backend-persisted settings notify the Settings page.
...
def test_best_of_both_guards_the_button_before_the_scan():
    ...
    with patch(
        "waves.desktop.backend._build_merge_plan",
...
def test_recycled_album_rows_reset_their_selection():
    ...
    # AlbumBlock is its own file, so the close is matched
