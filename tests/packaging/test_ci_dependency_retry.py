...
unsigned draft: that is what happened to v0.1.12. A bare ``uv sync`` here is
therefore a release-level single point of failure, and this guard keeps the
retry from being quietly simplified away.
"""
...
ACTION = REPO_ROOT / ".github/actions/setup-env/action.yml"
...
def _install_step() -> dict:
    ...
    assert step, "setup-env no longer has an 'Install dependencies' step"
    return step


def test_the_dependency_install_is_retried():
    script = _install_step()["run"]
    assert "uv sync" in script, "the step no longer installs anything"
...
def test_a_retry_does_not_reuse_a_truncated_download():
    """A short read can leave a partial wheel in uv's cache, so retrying
    without clearing it just fails again on the same bad file."""
    assert "uv cache clean" in _install_step()["run"]
