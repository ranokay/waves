"""The self-updater's asset selection + extraction.

``_INSTALL_EXTS`` must advertise only formats
``_extract_payload`` can actually unpack: a ``.dmg``/``.tar.gz`` ranked
equal-priority to ``.zip`` wins on the alphabetical
tie-break, is copied raw and executed (a bricked install).

Selection and extraction stay consistent: only formats the updater can
actually unpack+install are advertised (``.zip``, ``.exe``, ``.appimage``,
``.tar.gz``/``.tgz``); ``.tar.gz`` is extracted with the same path-traversal
...
def test_dmg_no_longer_preferred_over_zip(tmp_path):
    # .dmg must not rank installable: it beats .zip alphabetically and the
    # updater would pick an archive it cannot unpack. A .zip in the same
    # release wins.
    assets = [
