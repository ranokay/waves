"""The self-updater and the bundle size report.

Every case is about what the updater does to files it did not
create: a backup kept BECAUSE it held the user's own files must not be reclaimed
by the next update; a staged single-file swap must be resumable by a launch; a
Windows pid must not be handed out twice; a build's own dropped files must not
count as the user's; and an
install must not report a version it had not staged. Plus one build-script
pipeline that must not fail the whole build once the bundle grows.
...
def test_the_next_macos_update_does_not_delete_the_backup_this_one_kept(tmp_path, monkeypatch):
    """The keep is a promise the next update must not break.
    ...
    A recursive delete there would take the files the app said it
    had kept the moment another release shipped, silently and with no
