...
SearchProviderGroup.qml) that opts into the pixmap cache (``cache: true``)
must request its pixels the same way, so that one warmed entry serves all of
them. A new art surface that crops differently, or a pool that stops
...
# #315 slice 3), PreviewArt.qml (the track disc, split out in slice 5) and
...
COVER_FILES = (QML, ART_QML, PREVIEW_ART_QML, SEARCH_GROUP_QML)
