"""Hermetic tests for the WavesBridge (backend.py) guards.

Hermetic and Qt-free: constructing a real ``WavesBridge`` needs a Qt app, a
tidalapi session and a synchronous token-login network call, none of which
belong in a unit test. Each ``WavesBridge`` method under test here is plain
Python once its Qt signals are stubbed, so we bind the *real, unbound* method
onto a minimal ``_Stub`` carrying exactly the attributes that method touches
(fake signals record their emits). This exercises the shipped code path
without a running event loop.

Covered:
  * cancelQueueItem must NOT globally release the pause gate
  * clearQueue must abort every removed non-running item
  * logout must bump the library generation
  * best-of-both merge plan is kept on failure, dropped on success
  * loadMoreLibrary keeps a category scrollable after a transient error
  * _restore_ffmpeg_path clears an injected path but keeps a user override
"""
...
# --------------------------------------------------------------------------- #
# cancelQueueItem must not resume all paused workers.
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# clearQueue must not let a removed queued row download invisibly.
...
def test_logout_bumps_lib_gen_and_clears_cache():
    ...
    # The rest of the per-source shelves: a warm Home landing,
...
def test_stale_lib_gen_guards_cache_write_semantics():
    ...
    # in one turn must not cancel each other.
...
# --------------------------------------------------------------------------- #
# a failed best-of-both merge keeps its plan (retryable as a merge).
# --------------------------------------------------------------------------- #
...
def test_retry_reuses_stashed_merge_plan_for_album():
    ...
    def fake_download(obj, type_media, name, template, collection, media_id, merge_plan=None, keep_ask=None, **kwargs):
...
# --------------------------------------------------------------------------- #
# a transient loadMoreLibrary error must not exhaust the category.
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# restore-before-save clears an injected ffmpeg path but keeps a
...
def test_page_cache_round_trip_and_account_guard(tmp_path):
    ...
    # A second source's shelves round-trip as their own: same
