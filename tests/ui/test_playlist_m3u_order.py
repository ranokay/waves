"""The m3u8 lists a playlist's tracks in the playlist's own order.

Globbing the finished directory and sorting by filename (or file age)
reconstructs an album's order from numbered filenames but knows nothing of a
playlist's: a 300-song playlist would land in whatever order the names happened
to sort. The collection download hands the writer its landed file paths in list
order, and the playlist reproduces that order verbatim.
...
class TestTheM3uNeverShrinks:
    ...
    def test_a_redownload_that_skips_what_you_own_keeps_the_whole_list(self, tmp_path):
        # The shape pinned: 3 tracks on disk, one fetched this run (the
