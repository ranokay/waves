"""The bridge's half of the held-download path.

A download waiting for an unreachable download folder to come back must not
outlive every press that ends a download, nor lose the very state its hold
was taken to protect. Each test below states the user-visible sequence it
prevents.

  1  STOP, CANCEL and the clears must not leave a held download to start itself again
  2  a held REDOWNLOAD must not come back unforced or skip every file
  3  abandoning a hold must not leave its best-of-both plan for a later plain click
  4  a clear must not credit a failure against a row that just started running
  5  the motion-background sweep must not delete whatever else is in its folder
  6  a saturated pool must not report as idle in every line of a diagnostics export
"""
...
def test_a_stop_during_the_probe_keeps_its_stopped_row():
    """STOP keeps every stopped row in its own section with RETRY.
    Withdrawing the hold would delete the row the press just marked,
