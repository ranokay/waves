...
def test_content_markers_inside_a_title_cannot_end_the_span(redactor):
    ...
    for needle in ("Nothing » Everything", "Je t'aime «moi non plus", "x" * 900):
        line = f"queued {redactor.content(needle)} n=1"
        full = redactor.scrub(line, redact_content=True)
        assert "Everything" not in full and "moi non plus" not in full and "xxxx" not in full, needle
        assert "n=1" in full


def test_a_share_mount_point_never_survives(redactor):
    ...
    origin."""
    redactor.register_secret("/Volumes/CarolsMediaShare", "‹mount-point›")
    out = redactor.scrub("could not write /Volumes/CarolsMediaShare/Music/Artist/Album/01 Song.flac")
