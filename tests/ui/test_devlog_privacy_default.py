"""Tests for devlog's privacy-safe default.
...
def _load_devlog(monkeypatch, *, waves_debug=None, compiled=False, frozen=False):
    ...
    sys.modules.pop("waves.desktop.devlog", None)
    module = importlib.import_module("waves.desktop.devlog")
    ...
    if compiled:
        ...
        # the formula: that asserts the test's arithmetic against the fixture's
        # environment and cannot fail whatever devlog does, so a compiled-build
        # term added to the default would keep every one of these green while
        # shipping activity logging on by default in every real build.
        # test_the_default_stays_off_in_every_packaged_build below is the guard
        # that actually holds that line, by running devlog's own source under
        # the marker and asserting the VALUE.
...
def _enabled_in(markers: dict, waves_debug=None):
    ...
    namespace = {"__name__": "waves.desktop.devlog_probe", "__file__": module.__file__, **markers}
...
def test_disabled_event_stays_off_disk(monkeypatch, tmp_path):
    ...
    for lg in shared:
        for h in list(lg.handlers):
            lg.removeHandler(h)
    try:
        devlog = importlib.import_module("waves.desktop.devlog")
