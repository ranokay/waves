"""Rollup stranding and transient failures cached as authoritative.
...
# ---- a partially failed artist page is never cached -------------
...
# ---- a failed track fetch must not wipe learned membership ------
...
# ---- favourites pagination and failure caching ------------------
...
class _FavStub:
    ...
    def __init__(self, favorites):
        self._fav_ids: dict = {}
        # The pagination lives in TidalProvider; the stub
...
# ---- a partial discography scan must refuse to act --------------
...
class _DownloadArtistStub:
    ...
    def __init__(self):
        # The capabilities the sweep's gate reads: TIDAL answers
...
def test_download_artist_refuses_an_apple_artist():
    """The Apple discography sweep is the one verb the Apple
    catalog does not answer, and its refusal says so -- no future-tense
