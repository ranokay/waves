"""The legacy engine entry points' update-check behaviour.

Covers the update-check rules in ``waves.__init__``:

* A failed check (no network / bad response) reports **no** update
  available: ``update_available()`` must not return ``True`` for any
  non-``v0.0.0`` current version, which would surface a bogus
