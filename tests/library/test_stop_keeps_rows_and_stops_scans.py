"""STOP ends a discography scan in flight, keeps the rows it stops, and the
sweep's edition handling follows 'Most-complete edition only'.

WHAT THIS FENCES OFF
--------------------
Three behaviours, each against the real bridge method run over a stub carcass
(the same pattern as test_discography_video_source.py):
...
1. STOP during a discography scan. stopAll clears the queue, but a scan still
   ...
   so nothing stopAll touches reaches it: without a stop check the scan would
   finish after the press and queue the whole discography behind it, with the
   artist button stuck at "running". The scan checks the generation stopAll
   bumps at every hop that costs a request, and a stale scan drops what it
   gathered.

2. STOP keeps the rows. Emptying the queue on stopAll would make a press over
   one wrong item cost every other row and leave no record of what was in
   flight. Every queued and running row stays, marked cancelled, in a Stopped
...
3. The sweep with 'Most-complete edition only' OFF downloads every edition
   whole. Running 'Best of both' on the sweep regardless would leave a Standard
   beside its Deluxe as one merged album, and collapse a group whose merge
   declined anyway: the switch would be dead while on the page. Single-album
   clicks keep merging on their own (downloadAlbum's scan is not touched here;
   tests/metadata/test_edition_merge_gate.py covers it).
"""
...
def test_a_stop_check_is_stale_only_after_a_bump():
    ...
    from waves.desktop.backend import _ScanStopped
...
def test_the_help_says_the_sweep_follows_the_switch():
    # The control stays visible (hiding it would silently merge editions); the
