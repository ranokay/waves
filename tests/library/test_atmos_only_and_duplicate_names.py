"""Atmos-only tracks and colliding filenames.

With "Download Dolby Atmos" off, an Atmos-only track (an album's separate
Atmos edition, its own track id whose only audio mode is Dolby Atmos) must be
skipped outright: downloading it through the normal session delivers an AC-4
file the user asked not to have.

skip_existing must key on ids, not filenames: distinct tracks whose sanitized
names collide (several mixes sharing one title) would otherwise be skipped
after the first download. Files carry a WAVES_TIDAL_ID tag and the skip
decision compares ids: same id skips, a different id downloads under a
uniquified name, an untagged occupant (pre-tag library) keeps the skip so
re-fetching an old library cannot duplicate it.

The unique name is picked seconds before the file reaches it, so two colliding
tracks downloading side by side can both pick it and one overwrites or
silently loses the other. Names are claimed at the moment they are picked, and
the numbered-copy scan reads every variant that exists (gaps included), an
untagged one among them meaning identity unknown.
"""
...
from waves.metadata.tags import ITEM_ID_TAG, Metadata, read_item_id
from waves.paths import path_file_uniquify
...
class TestSkipExistingComparesItemIds:
    ...
    def test_a_gap_in_the_numbering_does_not_hide_the_copy(self, tmp_path):
        # The user removed Song_01 and kept Song_02. Stopping at the first gap
        # would make id 456 look absent and download all over again.
        dl = _make_download(skip_existing=True)
        (tmp_path / "Song.flac").write_bytes(b"x")
...
class TestCollidingNamesUnderConcurrency:
    """Two same-name tracks downloading side by side must not collide.
    ...
    picker also claims it. Without the claim both tracks pick the same name:
    one overwrites the other, or loses its own finished download without a word.
    """
    ...
    def test_two_colliding_tracks_both_land(self, tmp_path):
        ...
        # Both threads sit between picking the name and moving the file, the
