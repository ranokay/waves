...
def test_turning_apple_off_stops_its_queue_and_says_how_many():
    stub = _apply_stub(apple_enabled=True)
    calls = []
    stub._stop_provider_queue = lambda provider, reason: calls.append((provider, reason)) or 3
    status: list = []
...
def test_turning_apple_on_does_not_stop_the_queue():
    stub = _apply_stub()
    calls = []
    stub._stop_provider_queue = lambda provider, reason: calls.append((provider, reason)) or 0
