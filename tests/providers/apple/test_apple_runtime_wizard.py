...
# The status row's action pills, built by the same helper the schema uses:
# the setup management pills always, sign-out only while a session stands.
_SETUP_PILLS = [
...
_SIGN_OUT_PILL = {"label": "Sign out", "action": "apple_signout"}
...
def test_fresh_machine_light_is_not_set_up(tmp_path):
    stub = _bridge_stub(tmp_path, enabled=True, cookies="")
    assert stub.appleStatus() == {"state": "not_set_up", "word": "Not set up", "actions": _SETUP_PILLS}
...
def test_cookies_plus_binary_unlocks_signed_in_without_container(tmp_path):
    ...
    assert stub.appleStatus() == {"state": "signed_in", "word": "Signed in", "actions": [*_SETUP_PILLS, _SIGN_OUT_PILL]}
...
def test_live_flags_report_an_expired_session_and_recover(tmp_path, monkeypatch):
    ...
    assert stub.appleStatus() == {"state": "needs_attention", "word": "Needs attention", "actions": _SETUP_PILLS}
...
def test_managed_runtime_alone_is_runtime_ready(tmp_path, monkeypatch):
    ...
    assert stub.appleStatus() == {"state": "runtime_ready", "word": "Runtime ready", "actions": _SETUP_PILLS}
