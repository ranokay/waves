...
**Defaults** (amended 2026-09-18 to the shipped, ratified set — fresh installs only; existing installs keep their stored values): `lyrics_embed` off, `lyrics_file` **on**, `synced_only` off, `prefer_lrclib` on, word-timed **on**, `.ttml` sidecar **on** (Apple only; inert where the provider's engine serves no TTML). The best-quality-out-of-the-box set was ratified by issue #59 (commit `7dd5742`) after this section was written, so the older "off" lines here were stale, not a defect. Every lyrics/art key additionally has a per-provider mirror under the provider's card; the shared keys are one-time migration carriers (`provider_setting` reads the mirror first), and a fresh install starts every mirror at these values.
