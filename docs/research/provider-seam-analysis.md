...
| `waves/metadata.py` (tag writer)                                                                                                         | **Reusable as-is**                  | `Metadata` is a plain container (metadata.py:135-234); only its tag _names_ encode TIDAL identity (see Ownership seam).                                                                                        |
...
| `waves/waves_ui/backend.py:4384-4426` `_playlist_dict` / `_folder_dict`                             | playlist attrs + TIDAL playlist _folders_ (kind "folder", `path`, `plCount`).                                                                                                                                                                                                                |
...
| `waves/helper/tidal.py:265-309`                                                                     | `user_media_lists` — favorites playlists + playlist _folders_ + mixes categories.                                                                                                                                                                                                            |
...
3. **Three cross-cutting vocabularies must be shared or generalized:**
   item ids (ownership rows + WAVES*TIDAL*\* tags + `_objs` buckets),
...
4. **Biggest coupling surprises found:**
   - `metadata_write` (download.py:3772) is the _implicit_ provider fact
   ...
   - Quality pinning is stored as a _plain tier string_ on queue rows
