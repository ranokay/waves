...
| yt-dlp           | Floored to gamdl's own floor; the lockfile holds the version         | Normally only with a gamdl bump, or standalone for a security advisory | `uv lock` after a gamdl change; standalone advisory via `uv lock --upgrade-package yt-dlp` without moving the floor past gamdl's Requires-Dist (lockstep test below), same gates, note the advisory ID in the PR.                                                                    |
...
The declared floor must equal gamdl's `Requires-Dist` floor (`tests/packaging/test_ci_hygiene.py: test_yt_dlp_floor_matches_gamdl_and_exclusion_is_real` fails on drift, and fails if the locked yt-dlp drops `yt_dlp.extractor.lazy_extractors`).

Security exception: a yt-dlp security advisory is a valid standalone bump reason — run `mise run check`, `mise run test-strict` and the live account suite, and note the advisory ID in the PR.
