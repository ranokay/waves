...
| yt-dlp           | Floored to gamdl's own floor; the lockfile holds the version                                                                                                                    | Only with a gamdl bump, or standalone for a security advisory | `uv lock` after the gamdl change; never bump it alone except per the security exception below (same gates, note the advisory ID in the PR). |
...
| FFmpeg           | The FFmpeg manager's sources (martin-riedl, BtbN); accepted posture: same-origin checksum is integrity-only, macOS codesign advisory, HTTPS + smoke test (review by 2027-03-24) | On breakage, or the review date                               | Bump the manager's parser/pin when a source changes shape; smoke-test a managed install.                                                    |
...
Security exception: a yt-dlp security advisory is a valid standalone bump reason — run the same gates (`mise run check`, full suite, live account suite) and note the advisory ID in the PR.
